...
class PhonesPage {
  constructor({element}) {
    ...
    this._phonesCatalog = new PhonesCatalogue({
  ...
  _query({target: {value}}) {
    ...
    this._responseQuery = this._getPhones().filter(({name}) =>
        name.toUpperCase().match(query),
    );

    this._phonesCatalog._render(this._responseQuery);
  }

  _sorting(value) {
    const phones = this._responseQuery ? this._responseQuery : this._getPhones();
    let sortedPhones;
    ...
    if (value !== this._sortBy) {
      ...
      this._phonesCatalog._render(sortedPhones);
    }
  }
